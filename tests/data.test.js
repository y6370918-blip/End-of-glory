...
test("the private hand uses PUG-size cards and the enlarged map width", () => {
	const css = fs.readFileSync(path.join(root, "play.css"), "utf8")
	const html = fs.readFileSync(path.join(root, "play.html"), "utf8")
	assert.match(css, /#hand-panel\s*\{[^}]*width: calc\(var\(--map-width\) - 24px\)/s)
	assert.match(css, /#hand\s*\{[^}]*min-height: 364px/s)
	assert.match(css, /\.card-thumb\s*\{[^}]*flex: 0 0 250px[^}]*width: 250px[^}]*height: 340px/s)
	assert.doesNotMatch(css, /\.card-thumb\s*\{[^}]*width: 128px/s)
	assert.match(html, /<section id="hand-panel"[^>]*>\s*<div id="hand-title" class="panel-head">手牌<\/div>\s*<div id="hand"><\/div>/s)
...
test("the private MO tray follows the hand and uses stable full-size counters", () => {
	...
	assert.match(
		html,
		/<section id="hand-panel"[\s\S]*?<\/section>\s*<section id="mo-panel"[^>]*hidden[^>]*>\s*<div id="mo-list"><\/div>/
	)
...
test("map activation uses a PUG-style local menu and counter size classes", () => {
	...
	assert.match(play, /<body class="eog bevel">/)
...
test("PUG toolbar exposes one chat entry and the EOG utility menus", () => {
	const play = fs.readFileSync(path.join(root, "play.html"), "utf8")
	const client = fs.readFileSync(path.join(root, "play.js"), "utf8")
	const css = fs.readFileSync(path.join(root, "play.css"), "utf8")
	for (const id of ["stack-menu", "info-menu", "supply-menu", "track-button", "piece-button"])
	...
	assert.match(css, /body\.eog #log_button\s*\{[^}]*display: none/s)
})

test("game information uses independent PUG-style windows and mirrored counters", () => {
	const play = fs.readFileSync(path.join(root, "play.html"), "utf8")
	const client = fs.readFileSync(path.join(root, "play.js"), "utf8")
	const css = fs.readFileSync(path.join(root, "play.css"), "utf8")
	for (const id of ["overview-window", "ap-cards-window", "cp-cards-window", "unit-pools-window", "track-window"])
		assert.match(play, new RegExp(`id="${id}"[^>]*class="info-window`))
	assert.doesNotMatch(play, /id="info-dialog"/)
	assert.match(client, /function initializeInfoWindows\(\)/)
	assert.match(client, /function renderOverviewInfo\(\)/)
	assert.match(client, /function renderCardInfo\(faction\)/)
	assert.match(client, /function renderUnitPoolsInfo\(\)/)
	assert.match(client, /function renderTrackInfo\(\)/)
	...
	assert.match(client, /currentPieceScene\?\.units\.get\(unit\.id\)/)
	assert.doesNotMatch(client, /pieceElements\.get\(unit\.id\).*append/s)
	assert.match(css, /\.info-piece\.lcu\s*\{[^}]*width: 113px;[^}]*height: 113px;/s)
	assert.match(css, /\.info-piece\s*\{[^}]*width: 90px;[^}]*height: 90px;/s)
})
...
test("faction card information uses the compact PUG catalog and fixed card preview", () => {
	...
	const client = fs.readFileSync(path.join(root, "play.js"), "utf8")
	const css = fs.readFileSync(path.join(root, "play.css"), "utf8")
	for (const id of ["ap-cards-window", "cp-cards-window"])
		assert.match(play, new RegExp(`id="${id}"[^>]*info-window-card`))
	...
	assert.match(css, /\.info-window-card\s*\{[^}]*width: 350px;[^}]*height: 520px;/s)
	assert.match(css, /#info-card-tooltip\s*\{[^}]*right: 260px;/s)
})

test("turn record renders structured action slots and read-only snapshot navigation", () => {
	const client = fs.readFileSync(path.join(root, "play.js"), "utf8")
	...
	assert.match(client, /grid\.className = "track-grid"/)
	assert.match(client, /view\.action_history \|\| \[\]/)
	assert.match(client, /window\.request_snap/)
	assert.match(client, /function goToTrackTurn\(targetTurn\)/)
	assert.match(client, /无结构化记录/)
})
...
test("combat cards use two columns with an optional retained row", () => {
	const play = fs.readFileSync(path.join(root, "play.html"), "utf8")
	const client = fs.readFileSync(path.join(root, "play.js"), "utf8")
	const css = fs.readFileSync(path.join(root, "play.css"), "utf8")
	assert.match(play, /id="combat-cards-retained-zone"[^>]*hidden/)
	assert.match(client, /retainedZone\.hidden = retained\.childElementCount === 0/)
	assert.match(css, /#combat-card-zones\s*\{[^}]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/s)
	assert.match(css, /\.combat-card-zone-retained\s*\{[^}]*grid-column: 1 \/ -1/s)
	assert.match(css, /\.combat-card-row:empty::before\s*\{[^}]*content: attr\(data-empty-label\)/s)
	assert.match(css, /\.combat-card\s*\{[^}]*width: 250px;[^}]*height: 340px;/s)
})
...
test("Historical units are explicitly placed by the scenario in rules.js", () => {
	assert.equal(data.setup.historical.setup_source, "rules.js")
	assert.equal(data.setup.historical.objects, undefined)
	const rules = fs.readFileSync(path.join(root, "rules.js"), "utf8")
...
test("rules.js setup is isolated from normal builds and guarded against TTS regeneration", () => {
	...
	for (const script of ["seed_source_data.mjs", "freeze_setup_locations.mjs"]) {
		const source = fs.readFileSync(path.join(root, "tools", script), "utf8")
		assert.match(source, /setup_source === "rules\.js"/)
	}
})
