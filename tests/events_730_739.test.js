...
test("735 Below restrictions follow only the generated army and HQ instances", () => {
	...
	assert.deepEqual(
		operation.units.filter((entry) => entry.to === "map").map((entry) => entry.piece),
		["component-166", "component-087", "component-167"],
	)
})
...
test("737 Hindenburg-Ludendorff adds one MO and pays the second Russian step with selectable units", () => {
	...
	state.units = []
	state.reserves.cp = [
		...Array.from({ length: 5 }, (_, index) => unit(`ge-${index}`, "ge", "army", undefined)),
		unit("ah-half", "ah", "army", undefined),
	]
