...
function finishPendingEvent(state, role, card) {
	let guard = 0
	while (state.state === "event" || (state.state === "sr" && state.sr?.resume_event)) {
		if (guard++ > 50)
			throw new Error(
...
test("combat-card conditions are enforced and opponent commitments remain hidden", () => {
	...
	assert.equal(rules.view(state, "Central Powers").actions.combat_card?.includes(753) || false, false)
	...
	assert.equal(rules.view(state, "Central Powers").actions.combat_card.includes(753), true)
	state.hands.cp = []
...
test("Rupprecht requires an adjacent HQ and first fire removes casualties before return fire", () => {
	...
	assert.deepEqual(rules.view(state, "Central Powers").actions.combat_card || [], [])
	...
	assert.ok(rules.view(state, "Central Powers").actions.combat_card.includes(710))
...
test("post-combat cards are illegal before results and open server-driven repair choices afterwards", () => {
	...
	state.hands.ap = [631]
	configureCombatCardFixture(state, card)
	assert.equal(rules.view(state, "Allied Powers").actions.combat_card?.includes(631) || false, false)
