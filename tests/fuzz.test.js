	...
	while (state.state !== "game_over" && state.turn < targetTurn && steps++ < stepLimit) {
		...
		if (state.pending_event?.kind === "mo_penalty" && state.pending_event.stage === "loss")
			name = names.includes("confirm_mo_penalty_loss")
				? "confirm_mo_penalty_loss"
				: names.includes("select_mo_penalty_unit")
					? "select_mo_penalty_unit"
					: "deselect_mo_penalty_unit"
				...
				: names.includes("select_retreat_unit")
					...
					: names.includes("select_retreat_one")
						...
						: names.includes("select_retreat_two")
							? "select_retreat_two"
							: names.includes("decline_retreat")
								? "decline_retreat"
		...
		else if (state.state.startsWith("event_") && names.includes("event_units_confirm"))
			name = "event_units_confirm"
		else if (state.state.startsWith("event_") && names.includes("select_event_unit"))
		...
		assert.ok(
			names.includes(name),
			`seed ${seed} selected unavailable action ${name} in ${state.state}; available=${JSON.stringify(names)}`
		)
...
test("legal-action fuzz remains deterministic for the same seed", () => {
