...
test("a forged attack requires unused attack-activated units and unique ids", () => {
	...
	assert.ok(state.combat_window || state.combat || state.pending_retreat || state.combat_modifiers)
})
...
test("an activated space receives only one entrench attempt", () => {
	...
	const undoBeforeEntrench = state.undo.length
	rules.action(state, AP_ROLE, "entrench", space)
	assert.deepEqual(state.ops.entrench_attempted, [space])
	assert.equal(state.undo.length, undoBeforeEntrench + 1)
	...
	assert.deepEqual(state.ops.entrench_attempted, [])
	assert.equal(state.trenches[space] || 0, 0)
	rules.action(state, AP_ROLE, "entrench", space)
...
test("an LCU in the Italian theater constructs two points without adding SCU points", () => {
	const state = opsState(908)
	const models = {
		army: data.pieces.find((piece) => piece.nation === "it" && piece.type === "army" && !piece.veteran),
		corps: data.pieces.find((piece) => piece.nation === "it" && piece.type === "corps" && !piece.veteran),
	}
	assert.ok(models.army)
	assert.ok(models.corps)
	const units = [
		{ id: "it-army", piece: models.army.id, faction: "ap", nation: "it", type: "army" },
		{ id: "it-corps", piece: models.corps.id, faction: "ap", nation: "it", type: "corps" },
	]
	for (const unit of units) Object.assign(unit, {
		location: "udine", reduced: false, moved: false, attacked: false, supplied: true,
	})
	const card = data.cards.find((candidate) => candidate.id === 608)
	const rule = data.card_effects[608].operations.find((operation) => operation.type === "rule_modifier")
	state.turn = 4
	state.commitment.ap = "limited"
	state.units = units
	state.activations.udine = "construct"
	state.events[card.event] = { faction: "ap", rule }
	state.state = "ops_construct"
	state.ops.execution_phase = "construct"
	state.ops.execution_origin = "udine"
	state.ops.activated_units = { udine: units.map((unit) => unit.id) }
	rules.action(state, AP_ROLE, "entrench", "udine")
	assert.equal(state.fortifications.udine, 2)
})
...
test("a retreating unit cannot be voluntarily eliminated while a route exists", () => {
	...
	const retreatView = rules.view(state, AP_ROLE)
	assert.deepEqual(retreatView.actions.eliminate || [], [])
	assert.ok((retreatView.actions.retreat_destination || []).length > 0)
