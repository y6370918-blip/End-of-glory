...
test("units that cannot accept replacement points cannot pay an MO loss", () => {
	const state = setupGame(215)
	state.units = []
	state.reserves.ap = [
...
test("an unfulfilled MO never downgrades two forced markers to one", () => {
	...
	assert.equal(rules._test.beginMoPenaltyResolution(state), false)
	assert.notEqual(state.pending_event?.kind, "mo_penalty")
})
