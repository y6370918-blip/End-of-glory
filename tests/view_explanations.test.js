...
test("four public supply states have unambiguous precedence", () => {
	assert.equal(ViewExplanations.supplyStatus({ supplied: true }), "full")
	assert.equal(ViewExplanations.supplyStatus({ supplied: false, limited_supply: true }), "limited")
...
test("supply explanations list the concrete rule consequences", () => {
	assert.deepEqual(
		...
		["attack_column", "movement_attack", "construction", "replacement_points"]
	)
	assert.deepEqual(
		ViewExplanations.supplyEffects({ supplied: false, limited_supply: true }).map((entry) => entry.code),
		["attack_column", "movement_attack", "construction", "replacement_points"]
	)
	assert.deepEqual(
