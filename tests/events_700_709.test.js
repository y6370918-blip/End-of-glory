...
test("700 Race to the Sea counts distinct round-start enemy spaces across the whole action round", () => {
	const state = setupGame(700)
	state.round_start_control = { a: "ap", b: "ap", c: "ap" }
	state.round_enemy_entries = { cp: [], ap: [] }
	rules._test.recordRoundEnemyEntry(state, "cp", ["a", "b", "a"])
	assert.deepEqual(state.round_enemy_entries.cp.sort(), ["a", "b"])
	assert.equal(rules._test.enteredRoundStartEnemySpaces(state, "cp", ["c"]), 3)
	assert.equal(rules._test.roundEntryLimit(state, "cp"), 2)
	...
	assert.equal(rules._test.roundEntryLimit(state, "cp"), 3)
})
