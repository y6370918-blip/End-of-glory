...
test("fort-limited supply no longer adds an activation surcharge", () => {
...
test("German Italy units activate with either German or Austro-Hungarian units without a surcharge", () => {
	const state = rules.setup(14071)
	state.active = "cp"
	state.ops = { remaining: 6 }
	state.units = [
		{ id: "ge", piece: "component-033", faction: "cp", nation: "ge", type: "army", location: "bozen" },
		{ id: "ah", piece: "component-016", faction: "cp", nation: "ah", type: "army", location: "bozen" },
	]
	assert.equal(rules._test.activationCost(state, "bozen", "move"), 2)
	...
	assert.equal(rules._test.activationCost(state, "bozen", "move"), 2)
	state.units = [state.units[1], combined]
	assert.equal(rules._test.activationCost(state, "bozen", "move"), 1)
	assert.deepEqual(rules._test.replacementKeys(combined), ["ge", "ah"])
	assert.deepEqual(
		rules._test.replacementKeys({ ...combined, piece: "component-166" }),
		["ge", "ah"],
	)
})
...
test("combination restores one reduced LCU and eliminates one equivalent-nationality SCU", () => {
	...
	const undoBeforeCombination = state.undo.length
	...
	assert.equal(state.undo.length, undoBeforeCombination + 1)
	rules.action(state, AP_ROLE, "undo")
	assert.equal(state.units.find((unit) => unit.id === army.id).reduced, true)
	assert.equal(state.units.some((unit) => unit.id === corps.id), true)
	assert.equal(state.eliminated.ap.some((unit) => unit.id === corps.id), false)
})
