...
for (const obsoleteSidecar of ["edge_rules.json", "setup.json"]) {
	if (fs.existsSync(path.join(sourceDir, obsoleteSidecar)))
		throw new Error(`${obsoleteSidecar} is obsolete; formal map data lives in edges.json and Historical setup lives in rules.js`)
}
