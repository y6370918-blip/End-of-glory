...
function mergeSpace(space, proposal) {
	...
	if (Object.hasOwn(proposal || {}, "vp")) {
		if (proposal.vp) merged.vp = true
		else delete merged.vp
	}
	return merged
}
...
export function auditMultiRegionScopeErrors(current, incoming, regionIds) {
	...
	for (const [id, record] of Object.entries(incoming.spaces || {})) {
		...
		if (before && changed && record.proposal && Object.keys(record.proposal).some((key) => !["ui", "vp"].includes(key))) errors.push(`现有地块只能修改点击框和VP属性：${id}`)
...
export function reconcileDraftManifest(formal, current, incoming, draft) {
	...
	for (const [id, space] of formalSpaces) {
		...
		const { ui: formalUi, vp: formalVp, ...formalRules } = space
		const { ui: draftUi, vp: draftVp, ...draftRules } = draftSpace
		if (!same(formalRules, draftRules)) errors.push(`现有地块只能修改点击框和VP属性：${id}`)
		const uiChanged = !same(formalUi, draftUi)
		const vpChanged = Boolean(formalVp) !== Boolean(draftVp)
		if (!uiChanged && !vpChanged) {
			next.spaces[id] = metadata(incoming.spaces?.[id], current.spaces?.[id])
			continue
		}
		const region = current.spaces?.[id]?.region
		const proposal = {}
		if (uiChanged) proposal.ui = clone(draftUi)
		if (vpChanged) proposal.vp = Boolean(draftVp)
		next.spaces[id] = metadata(incoming.spaces?.[id], {
			...current.spaces?.[id], region, status: "pending", decision: "update", proposal
		})
		next.spaces[id].region = region
		next.spaces[id].status = "pending"
		next.spaces[id].decision = "update"
		next.spaces[id].proposal = proposal
	}
