...
function syncSpaceProposal(space) {
	...
	if (same(space, formal)) {
		...
		if (formal) {
			record.proposal = {}
			if (!same(space.ui, formal.ui)) record.proposal.ui = clone(space.ui)
			if (Boolean(space.vp) !== Boolean(formal.vp)) record.proposal.vp = Boolean(space.vp)
		} else record.proposal = clone(space)
	}
...
function prepareSpacePlacement(event) {
	...
	if (form.elements.vp.checked) placingSpace.vp = true
...
function inspectSpace(space) {
	...
	const properties = newSpace ? [
		...
		booleanField("VP地区", Boolean(space.vp), (value) => {
			if (value) space.vp = true; else delete space.vp
			syncSpaceProposal(space)
		}),
